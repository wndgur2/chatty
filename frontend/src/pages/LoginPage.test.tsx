...
describe('LoginPage', () => {
  beforeEach(() => {
    ...
    useLoginSpy.mockReturnValue({ mutateAsync: mutateAsyncSpy, isPending: false })
  })
  ...
  it('submits trimmed username and navigates on success', async () => {
    render(<LoginPage />)
    fireEvent.change(screen.getByLabelText('Username'), { target: { value: '  jun  ' } })
    fireEvent.submit(screen.getByRole('button', { name: 'Sign in' }).closest('form')!)

    await waitFor(() => {
      expect(mutateAsyncSpy).toHaveBeenCalledWith({ username: 'jun' })
